from graph_embedding_rag.models import Node2VecModel
from graph_embedding_rag.embed_models import PrecomputedEmbedding
    ...
    def add_to_vector_store(self):
        ...
        for i in tqdm(range(0, len(nodes), batch_size), desc="Adding to vector store"):
            ...
            for node in batch:
                ...
                json_safe_props = json.loads(
                    json.dumps(node_data, cls=CustomJSONEncoder)
                )

                text = f"Node ID: {node_id}, Label: {json_safe_props['label']}, "
                text += ", ".join(
                    [f"{k}: {v}" for k, v in json_safe_props.items() if k != "label"]
                )
                ...
                metadata.update(
                    {k: v for k, v in json_safe_props.items() if k != "label"}
                )
